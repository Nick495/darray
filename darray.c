 ...
 * 	darray_init().
 *
 * TODO: Refactor to make darray->cap be the literal amount of bytes addressed.
...
static int darray_ensure_size(struct darray *d, const size_t val_size)
{
	...
	const size_t new_cap = d->cap * 2; /* Add sophistication as needed. */
	if (new_cap < d->cap) {
		rc = 1;
	...
	const size_t asize = new_cap * val_size;
...
static int darray_bmap_push(struct darray *d, const void *v)
{
	/* Preconditions */
	assert(d->use <= d->cap);

	int rc = 0;
	if ((rc = darray_ensure_size(d, sizeof(unsigned int)))) {
		goto fail_darray_ensure_size;
	}
	assert(d->use < d->cap);

	const size_t rind = d->use++;
#if NOSERIAL
	memcpy(&d->data[rind], v, sizeof(uint64_t));
#else
	d->data[rind + 0] = (*((const uint64_t *) v) >> 56);
...
int darray_init(struct darray *d, const size_t init_size, const size_t val_size,
		const char *dir, const char *name, const char *ext)
{
	...
	const size_t dir_len = strlen(dir); /* More preconditions */
	const size_t name_len = strlen(name);
	const size_t ext_len = strlen(ext);
	assert(name[name_len] == '\0');
	assert(dir[dir_len] == '\0');
	assert(ext[ext_len] == '\0');
...
void darray_free(struct darray *d, size_t val_size)
{
	/* Preconditions */
	assert(d->use <= d->cap);
	assert(val_size > 0);
...
#include <stdio.h>
void darray_print_all(struct darray *d)
{
	assert(d->path != NULL);
	assert(d->data != NULL);
	assert(d->use <= d->cap);
...
int main(void)
{
	int rc = 0;
	struct darray d;
	if ((rc = darray_init(&d, 0, sizeof(uint32_t),
